...
class UsageStatUI(Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.dbpath = 'D:/' # <------ path to save database file, same as in "LayoutTool/scriptUsage.py"
        ...
        self.export_btn = ttk.Button(self.func_frm, text='Export CSV', width = 15, command=self.exportCsv)
        ...
        self.table = ttk.Treeview(self.table_frm, column=col_ls, show='headings', selectmode='extended', yscrollcommand=self.scl_table.set)
        self.table.pack(fill='both', expand=True)

        for i in range(6):
            self.table.column('{}'.format(i), width=col_w_ls[i])
            self.table.heading('{}'.format(i), text=col_ls[i], command=lambda x = col_cmd[i]: self.generateTable(x))


        self.refresh()


        self.month_ls_cmb.bind('<<ComboboxSelected>>', lambda x: self.generateTable('id'))
        self.year_ls_cmb.bind('<<ComboboxSelected>>', lambda x: self.generateTable('id'))
    ...
    def getTableData(self):
        ...
        if month == 'All' and year == 'All':
            sqlcmd = """SELECT * FROM script_usage ORDER by {}, {}""".format(self.order, self.suborder)
        elif month == 'All' and year != 'All':
            sqlcmd = """SELECT * FROM script_usage WHERE strftime('%Y', timestamp) = '{}' ORDER by {}, {}""".format(year, self.order, self.suborder)
        elif month != 'All' and year == 'All':
            sqlcmd = """SELECT * FROM script_usage WHERE strftime('%m', timestamp) = '{}' ORDER by {}, {}""".format(month, self.order, self.suborder)
        else:
            sqlcmd = """SELECT * FROM script_usage WHERE strftime('%m-%Y', timestamp) = '{}-{}' ORDER by {}, {}""".format(month, year, self.order, self.suborder)
